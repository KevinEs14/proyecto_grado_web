...
/* /common/init.js*/
/**
 * @preserve Basic Primitives Diagram v4.1.0
...
(function () {
	...
	primitives.common.version = "4.1.0";
}());
...
/* /enums/QuadrantType.js*/
primitives.common.QuadrantType =
{
	TopLeft: 0,
	TopRight: 1,
	BottomLeft: 2,
...
primitives.famdiagram.ItemsPositionsTask = function (currentControlSizeTask, scaleOptionTask, orientationOptionTask, itemsSizesOptionTask, connectorsOptionTask, visualTreeOptionTask,
	...
	function setOffsets() {
		...
		var familyAlignment = new primitives.common.FamilyAlignment(this, family, treeLevels, function (nodeid, node) {
			var treeItemPosition = _data.treeItemsPositions[nodeid];
			var treeItemPadding = intervalsTask.getInterval(treeItemPosition.actualVisibility) / 2;
...
primitives.famdiagram.NormalizeLogicalFamilyTask = function (normalizeOptionTask, addSpousesTask) {
	...
	function process(debug) {
		...
		if (_normalizeOptions.hideGrandParentsConnectors == true) {
				...
				if (showCallout) {
					...
					_transform.transformRect(actualPosition.x, actualPosition.y, actualPosition.width, actualPosition.height, true,
						this, function (x, y, width, height) {
							...
							if ((treeItemPosition.actualVisibility >= _options.calloutMaximumVisibility && treeItemPosition.actualVisibility != 4/*primitives.common.Visibility.Invisible*/) || !panelPosition.overlaps(snapRect)) {
...
primitives.common.SpatialIndex = function (sizes) {
	...
	switch (sizes.length) {
		case 0:
		case 1:
			_buckets.push(new Bucket(0, 100000000));
			break;
		case 2:
			var size1 = sizes[0];
			var size2 = sizes[1];
	...
	function Bucket(minimum, maximum) {
		...
		this.quadTree = primitives.common.QuadTree(maximum);
	}
...
primitives.common.SortedList = function () {
	...
	function loopForward(thisArg, fromValue, onItem) { //function onItem(index, value, context)
		if (onItem != null) {
			var trace = [];
			var node = null;
			var next = _rootNode;
...
primitives.common.QuadTree = function (minimalSize) {
	var _minimalCellSize = minimalSize,
		_maximumPointsPerCell = 10,
		_startSize = 400,
		_rootCell = -1, // cell index in _cells hash
		_cells = [],
		_cellIndex = 0;
	...
	function Cell(rect) {
		this.quadrants = [];
		this.points = [];
		this.position = rect; // primitives.common.Rect
	}

	function addPoint(point) {
		if (_rootCell == -1) {
			var cell = new Cell(new primitives.common.Rect(point.x - _startSize, point.y - _startSize, _startSize * 3, _startSize * 3));
			_cells[_cellIndex] = cell;
			_rootCell = _cellIndex;
			_cellIndex += 1;
		} else {
			while (!_cells[_rootCell].position.contains(point)) {
				_nestPoint(point);
			}
		}
		_addPoint(point);
	...
	function _nestPoint(point) {
		var position = _cells[_rootCell].position;
		if (!position.contains(point.x, point.y)) {
			if (point.x < position.x) {
				if (point.y < position.y) {
					_nestQuadrant(3/*primitives.common.QuadrantType.BottomRight*/);
				} else {
					_nestQuadrant(1/*primitives.common.QuadrantType.TopRight*/);
				}
			} else {
				if (point.y < position.y) {
					_nestQuadrant(2/*primitives.common.QuadrantType.BottomLeft*/);
				} else {
					_nestQuadrant(0/*primitives.common.QuadrantType.TopLeft*/);
				}
			}
		}
	}

	function _nestQuadrant(quadrant) {
		var position = _cells[_rootCell].position;
		var rect = null;
		switch (quadrant) {
			case 0/*primitives.common.QuadrantType.TopLeft*/:
				rect = new primitives.common.Rect(position.x, position.y, position.width * 2, position.height * 2);
				break;
			case 1/*primitives.common.QuadrantType.TopRight*/:
				rect = new primitives.common.Rect(position.x - position.width, position.y, position.width * 2, position.height * 2);
				break;
			case 2/*primitives.common.QuadrantType.BottomLeft*/:
				rect = new primitives.common.Rect(position.x, position.y - position.height, position.width * 2, position.height * 2);
				break;
			case 3/*primitives.common.QuadrantType.BottomRight*/:
				rect = new primitives.common.Rect(position.x - position.width, position.y - position.height, position.width * 2, position.height * 2);
				break;
		}

		var cell = new Cell(rect);
		cell.quadrants = [];
		cell.quadrants[quadrant] = _rootCell;
	...
	function _addPoint(point) {
		var cell = _cells[_rootCell];
		while (cell.points == null || cell.points.length >= _maximumPointsPerCell) {
			var position = cell.position;

			if (cell.points != null && position.width > _minimalCellSize) {
				_splitCell(cell);
			}
			
			var quadrant = null;
			if (point.x < position.horizontalCenter()) {
				if (point.y < position.verticalCenter()) {
					quadrant = 0/*primitives.common.QuadrantType.TopLeft*/;
				} else {
					quadrant = 2/*primitives.common.QuadrantType.BottomLeft*/;
				}
			} else {
				if (point.y < position.verticalCenter()) {
					quadrant = 1/*primitives.common.QuadrantType.TopRight*/;
				} else {
					quadrant = 3/*primitives.common.QuadrantType.BottomRight*/;
				}
			}

			var cellIndex = cell.quadrants[quadrant];
			if (cellIndex == null) {
				cellIndex = _createQuadrant(cell, quadrant);
			}
	...
	function _splitCell(cell) {
		var position = cell.position,
			horizontalCenter = position.horizontalCenter(),
			verticalCenter = position.verticalCenter();

		// rearrange points
		for (var index = 0, len = cell.points.length; index < len; index+=1) {
			var point = cell.points[index];

			var quadrant = null;
			if (point.x < horizontalCenter) {
				if (point.y < verticalCenter) {
					quadrant = 0/*primitives.common.QuadrantType.TopLeft*/;
				} else {
					quadrant = 2/*primitives.common.QuadrantType.BottomLeft*/;
				}
			...
			var targetCell = _cells[cellIndex];
	...
	function _createQuadrant(cell, quadrant) {
		var position = cell.position,
			horizontalCenter = position.horizontalCenter(),
			verticalCenter = position.verticalCenter(),
		...
		switch (quadrant) {
			case 0/*primitives.common.QuadrantType.TopLeft*/:
				point = new primitives.common.Point(position.x, position.y);
				break;
			case 1/*primitives.common.QuadrantType.TopRight*/:
				point = new primitives.common.Point(horizontalCenter, position.y);
				break;
			case 2/*primitives.common.QuadrantType.BottomLeft*/:
				point = new primitives.common.Point(position.x, verticalCenter);
				break;
			case 3/*primitives.common.QuadrantType.BottomRight*/:
				point = position.centerPoint();
				break;
		}

		_cells[_cellIndex] = new Cell(new primitives.common.Rect(point.x, point.y, width, height));
		cell.quadrants[quadrant] = _cellIndex;
		_cellIndex += 1;
	...
	function validate() {
		for (var cellIndex = 0; cellIndex < _cells.length; cellIndex += 1) {
			var cell = _cells[cellIndex];
			if (cell.points != null) {
				for (var pointIndex = 0; pointIndex < cell.points.length; pointIndex += 1) {
					var point = cell.points[pointIndex];

					if (!cell.position.contains(point)) {
						return false;
					}
				}
			}
			...
			for (var quadrantIndex = 0; quadrantIndex < 4; quadrantIndex += 1) {
				var subCellIndex = cell.quadrants[quadrantIndex];
				if (subCellIndex != null) {
					var subCell = _cells[subCellIndex];

					if (!cell.position.overlaps(subCell.position)) {
						return false;
					}
				}
			}
