...
/* /Algorithms/Perimeter/PerimeterManager.Tests.js*/
...
/* /Algorithms/Algorithms.Tests.js*/
...
QUnit.test("primitives.common.QuadTree", function (assert) {
	...
	(function () {
		var quadTree = primitives.common.QuadTree(10);

		var point = new primitives.common.Point(100, 100);
		point.context = 1;
		quadTree.addPoint(point);

		var result = [];
		quadTree.loopArea(this, new primitives.common.Rect(50, 50, 100, 100), function (point) {
			result.push(point.context);
		});

		var expectedResult = [1];

		assert.deepEqual(result, expectedResult, "Quad Tree should return point added into it");
	})();
	...
	(function () {
		...
		for (var x = 0; x <= 1000; x += 100) {
			for (var y = 0; y <= 1000; y += 100) {
				quadTree.addPoint(new primitives.common.Point(x, y));
			}
		}

		assert.ok(quadTree.validate(), "Quad Tree internal structure should pass self validation");
	})();

	(function () {
		var area = new primitives.common.Rect(450, 550, 200, 100);
		var expectedResult = [];
		...
		for (var x = 0; x <= 1000; x += 100) {
			for (var y = 0; y <= 1000; y += 100) {
				var point = new primitives.common.Point(x, y);
				point.context = index;

				if(area.contains(point)) {
					expectedResult.push(index);
				}
		...
		var result = [];
		quadTree.loopArea(this, area, function (point) {
			result.push(point.context);
		});

		result.sort();
		
		assert.deepEqual(result, expectedResult, "Quad Tree should return points added into the area of selection");
	})();
});
...
QUnit.test("primitives.common.SpatialIndex", function (assert) {
	(function() {
		var spatialIndex = primitives.common.SpatialIndex([5, 16, 200]);

		var rects = [
			new primitives.common.Rect(100, 100, 5, 5),
			new primitives.common.Rect(210, 100, 16, 16),
			new primitives.common.Rect(310, 100, 200, 200)
		];
		...
		for (var index = 0; index < rects.length; index += 1) {
			...
			rect.context = index;
			spatialIndex.addRect(rect);

			expectedResult.push(index);
		}

		var result = [];
		spatialIndex.loopArea(this, new primitives.common.Rect(0, 0, 500, 500), function (rect) {
			result.push(rect.context);
		});

		result.sort();

		assert.deepEqual(result, expectedResult, "Spatial Index should return all added rectangles into it");
	})();
});
...
QUnit.module('FamDiagram - Tasks - Transformations - NormalizeLogicalFamilyTask - EdgeItem');
...
/* /Controls/FamDiagram/Tasks/Transformations/NormalizeLogicalFamilyTask.Tests.js*/
