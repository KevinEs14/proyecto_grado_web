...
class Index extends Table {
    ...
    function obtenerDisponibleCarne(){
        $q1="SELECT SUM(tcc.total_carne) as carneT,SUM(tcc.total_charque) as charqueT
            ...
            INNER JOIN usuario usu1 on
            usu1.itemId=rl1.usuario_itemId
            WHERE
        ...
        $q2="SELECT SUM(ac.entregado_carne_primera+ac.entregado_carne_segunda) as carneE,SUM(ac.entregado_charque) as charqueE
            ...
            usu2.itemId=rl3.usuario_itemId
            WHERE
            ac.estado!=0 and
    ...
    function obtenerTco(){
        ...
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
                $sql="SELECT  t1.itemId,u2.rol_itemId
                FROM  usuario u2,representante_legal t1
                LEFT JOIN usuario u on u.itemId=t1.usuario_itemId
                WHERE u2.itemId= ".$_SESSION["memberId"]." and (u.itemId=".$_SESSION["memberId"]." or u2.rol_itemId=1 or u2.rol_itemId=4)
                group by u2.rol_itemId"
                ;
        ...
        if($idItem!=''){
            if($tipoTabla=='tipo'){
                ...
                $sqlFrom = ' acta_carne i
                         ...
                         LEFT JOIN usuario u3 on u1.itemId=i.userCreate
                         LEFT JOIN usuario u2 on u2.itemId=i.userUpdate
                         ';
                $sqlWhere = ' i.itemId='.$idItem.' and (u3.rol_itemId=1 or u3.rol_itemId=3) and u3.itemId='.$_SESSION["memberId"];
    ...
    function item_delete($id){
        ...
        if($ver){
            $res = $this->item_delete_sbm($id,$campo_id,"acta_carne","estado=2");
        }
        return $res;
    }
