...
class Index extends Table {
    ...
    function get_item($idItem,$tipoTabla,$variante=""){
        ...
        if($idItem!=''){
            if($tipoTabla=='tipo'){
                $sqlSelect = ' i.*,t.itemId as tco_itemId,t.municipio_itemId,rcu.curtiembre_itemId
                ...
                $sqlFrom = ' usuario i
                         ...
                         LEFT JOIN usuario u3 on u2.itemId=i.userUpdate
                         LEFT JOIN cazador c on c.usuario_itemId=i.itemId
                         LEFT JOIN representante_legal rl on rl.usuario_itemId=i.itemId
                         LEFT JOIN responsable_curtiembre rcu on rcu.usuario_itemId=i.itemId
                         LEFT JOIN catalogo_tco t on (t.itemId=c.tco_itemId or t.itemId=rl.tco_itemId) ';
                $sqlWhere = ' i.itemId='.$idItem.' and u3.rol_itemId=1 and u3.itemId='.$_SESSION["memberId"];
                $sqlGroup = '';
            }else{
                $sqlSelect = ' i.*';
    ...
    function item_delete($id){
        $campo_id="itemId";
        $res = $this->item_delete_sbm($id,$campo_id,"usuario");
        return $res;
    }
