...
class Table
{
    ...
    function verificar_permiso_usuarios($rol_Ids){
        if(count($rol_Ids>0)){
            try{
                $sql = "select usuario.itemId from usuario where usuario.itemId=".$_SESSION["memberId"]." and (";
                for($i =0;$i<count($rol_Ids)-1;$i++){
                    $sql.=" usuario.rol_itemId=".$rol_Ids[$i]." or ";
                }            
                $sql.=" usuario.rol_itemId=".$rol_Ids[count($rol_Ids)-1].");";
                $resultado = $this->dbm->Execute($sql);
                $resultado = $resultado->GetRows();
                if(count($resultado)==0){
                    return false;
                ...
                echo "<script>console.log('".$this->dbm->errorMsg()."');</script>";
                return false;
            }
        }
        else{
...
//echo "<br><br>=>".$sql;
    ...
    function item_delete_sbm2($itemId,$campo_id,$tabla,$where=""){
        global $privFace;
 ...
 /**
    ...
    public function pentaho_get_report($reporte_file,$type,$nombre,$parametros_otros=""){
        ...
        echo"<script>alert('".$this->pentaho["url"].$this->pentaho["carpeta"].$reporte_file."/report"."')</script>";
        $parametro = array();
