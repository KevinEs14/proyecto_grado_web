<?php
 ...
 class core{
    ...
    public function get_datos_miga($module,$smodule){
         ...
         */
        $sql = "select * from  submodulo as sm where sm.carpeta ='".$smodule."' ";
        $smodulo =  $db->Execute($sql);
        $smodulo = $smodulo->fields;
        /**
         * Sacamos el padre del sub módulo
         */
        $sql = "select menu.catalogo_itemId from menu left join submodulo as sm on sm.itemId= menu.submodulo_itemId where sm.itemId = '".$smodulo["itemId"]."' ";
    ...
    function getUsuarioInfo($id){
        ...
        $sql = "select 
        u.usuario
        , u.ingresos,u.ingresos_fallidos,u.nombre,u.apellido, r.nombre_rol,r.itemId as rol_itemId
        from  usuario as u   
        INNER JOIN catalogo_rol as r
        on r.itemId=u.rol_itemId
        where u.itemId ='".$id."' ";
        //echo $sql;exit;
        $db->SetFetchMode(ADODB_FETCH_ASSOC);
        $item =  $db->Execute($sql);
   ...
   function getUserSubModulePrivileges($smodule){
    ...
    function get_menu_principal(){
        ...
        from catalogo as c left join catalogo as c2 on c2.itemId = c.padre 
        ...
        for ($i=0;$i<count($item);$i++){
            ...
            , m.*
            from menu as m 
            left join submodulo as sub on m.submodulo_itemId = sub.itemId
            left join catalogo_rol as r on r.itemId=m.rol_itemId
            left join usuario as u on u.rol_itemId=r.itemId
